...
- Current snapshot:
  ...
  - Current pass:
    ...
    - Direct source-compatible `COMCTL32.dll` imports now resolve for
      `InitCommonControls` / `InitCommonControlsEx` through packaged native
      shim assets, with focused UIIntegration coverage.
    - The same native `COMCTL32.dll` shim now covers first-tier ImageList
    ...
    - KERNEL32 first-tier memory state now belongs to the system PAL as well:
      direct `KERNEL32.dll` `GlobalAlloc` / `GlobalReAlloc` / `GlobalLock` /
      `GlobalUnlock` / `GlobalSize` / `GlobalFree` and matching `Local*`
      imports share deterministic WinFormsX zero-init, resize, size, lock, and
      free behavior.
    - KERNEL32 first-tier activation context state now belongs to the system
    ...
    - KERNEL32 basic thread/locale/startup state now belongs to the system PAL
      ...
      `GetLocaleInfoEx`, `GetStartupInfo`, `GetThreadLocale`, `GetTickCount`,
      `GetTickCount64`, `QueryPerformanceCounter`,
      `QueryPerformanceFrequency`, `GetACP`, `GetOEMCP`,
      `GetSystemDefaultLCID`, and `GetUserDefaultLCID` now share deterministic
      WinFormsX behavior.
    ...
    - KERNEL32 module-resource lookup now belongs to the system PAL as well:
      managed wrappers and direct `KERNEL32.dll` imports for `FindResourceW/A`,
    ...
    - Direct source-compatible `UXTHEME.dll` imports now resolve through a
    ...
    - Direct source-compatible `OLE32.dll` imports now resolve through a
      ...
      registration state, lock-bytes/stream helper creation
  ...
  - Full UIIntegration now completes without a hang/abort:
    ...
    - Latest task-dialog pass:
      ...
      button state, progress-bar range/value/state updates, and text/caption
      updates through the existing
      `TaskDialogPage` model. Focused `TaskDialogUITests` reports
      `Passed: 5, Failed: 0`; the focused dialog group reports
  ...
  - Priority order moves to remaining high-impact infrastructure gaps:
    ...
    output design, OS-native picker integration, KERNEL32 resource/loader
    breadth, then lower-volume accessibility/provider breadth and resource
    polish.
  ...
  - Active lane update: focused drag/drop input coverage has moved forward:
    ...
    - Latest create-prompt coverage:
      `SaveFileDialog.CreatePrompt` now has owner-driven automation coverage
      on the managed WinFormsX backend path. Focused
      `ManagedCommonDialogTests` now reports `Passed: 10, Failed: 0`.
    - Latest ColorDialog custom-color pass:
      `ColorDialog.CustomColors`, `AllowFullOpen`, and `FullOpen` now thread
      into the managed WinFormsX picker, and seeded custom colors are selectable
      through owner-driven automation. Focused `ManagedCommonDialogTests` now
      reports `Passed: 11, Failed: 0`.
    - Latest FontDialog color pass:
      `FontDialog.ShowColor` and `Color` now round-trip through the managed
      WinFormsX font picker, using the same owner-driven automation path as
      font/effect selection. Focused `ManagedCommonDialogTests` now reports
      `Passed: 12, Failed: 0`.
...
- In-progress local changes (next commit):
  - Added visible managed `TaskDialog` progress-bar materialization and update
    handling for range, position, marquee mode, and state messages.
  - Fixed a `System.Private.Windows.Core` multi-target build blocker by using
    a target-compatible global-memory lock object instead of a .NET 9-only
    lock type.
  - Verification:
    - Build `System.Private.Windows.Core`: succeeded.
    - Build `System.Windows.Forms.UI.IntegrationTests`: succeeded.
    - UIIntegration focused filter for KERNEL32, OLE32, GDI+, and TaskDialog
      progress: `Passed: 4, Failed: 0`.
- In-progress local changes (next commit):
  - Added a native `gdiplus.dll` compatibility facade for first-tier direct
  ...
  - Scope note: this is deterministic source-compatibility behavior only; it
    does not switch WinFormsX to host-native GDI+ drawing.
- In-progress local changes (next commit):
  - Expanded the native `GDI32.dll` facade with safe first-tier bitmap, DIB
    ...
    `CreateRectRgn`, and `CombineRgn`.
  - Verification:
    - UIIntegration filter
      `DirectGdi32DllImports_ResolveToWinFormsXFacade`: `Passed`.
    - Full `UIIntegrationTests`: `Failed: 0, Passed: 259, Skipped: 1, Total: 260`.
    - `WinformsControlsTest --control-smoke-test`: `total=42 passed=41 failed=0 skipped=1`.
- In-progress local changes (next commit):
  - Expanded the native `GDI32.dll` facade with the next source-compatibility
    breadth tier:
    - drawing/no-op-safe calls: `SelectObject`, `PatBlt`, `BitBlt`;
    - clipping calls: `SelectClipRgn`, `IntersectClipRect`, `GetClipBox`;
    - brush/palette calls: `CreateBrushIndirect`, `CreatePatternBrush`,
      `CreateHalftonePalette`, `SelectPalette`, `RealizePalette`,
      `GetPaletteEntries`;
    - print DC/document lifecycle calls: `CreateDCW/A`, `CreateICW/A`,
      `StartDocW/A`, `StartPage`, `EndPage`, `EndDoc`, `AbortDoc`,
      `ExtEscape`.
  - Verification:
    - Build `System.Windows.Forms.UI.IntegrationTests`: succeeded.
    - UIIntegration filter
...
- [~] WXA-1303: Implement fallback graphics path for `PrintDocument` and `PrintControllerWithStatusDialog` for integration tests. Basic `StandardPrintController` event flow now has an offscreen graphics fallback; the native `GDI32.dll` facade now resolves deterministic print DC/document lifecycle calls for source-compatible direct imports. Status dialog and real output provider work remain.
