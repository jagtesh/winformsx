...
- [~] WXA-1201: Implement managed fallbacks for `OpenFileDialog`, `SaveFileDialog`, `FolderBrowserDialog`, `ColorDialog`, `FontDialog`. Visible WinFormsX form baselines, focused owner-driven accept/cancel automation, and Open/Save wildcard filter application are covered; richer multi-select UI, overwrite/create prompts, font effects/scripts, custom-color state, and OS-native picker integration remain.
